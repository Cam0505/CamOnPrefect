...
def satellite_source(logger, prev_positions: dict):
    ...
    def satellite_resource():
        ...
        for i in range(0, len(lines), 3):  # Name, line1, line2
            ...
            dt = datetime.utcnow()
            jd, fr = jday(dt.year, dt.month, dt.day, dt.hour,
            ...
            if e != 0:
                ...
                logger.error(
                    f"SGP4 failed for satellite {name.strip()} with error code {e}.")
            ...
            if prev_data:
                dx, dy, dz = x - prev_data["x_km"], y - \
                    prev_data["y_km"], z - prev_data["z_km"]
        ...
        if not did_yield:
            logger.info(
                "🟡 No new or changed satellite data found — skipping load.")
            return
